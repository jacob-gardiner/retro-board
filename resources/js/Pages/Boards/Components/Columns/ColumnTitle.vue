...
<template>
    <div class="p-4">
        <form @submit.prevent="submit" v-if="isEditing">
            <TextInput @keyup.esc="toggle()" @focusout="isEditing = false" v-focus v-model="form.title" type="text"
                       class="w-full" data-testid="column-title-input"/>
        </form>

        <h3 v-else @click="toggle()"
